...
class AttendanceService {
  ...
  static async markAbsent(userId, notes = '') {
    ...
    if (attendance) {
      if (attendance.checkIn) throw new Error('Cannot mark absent: Already checked in.');
    } else {
      attendance = new Attendance({ userId, date: start });
    }
  ...
  static async approveAbsence(attendanceId) {
    ...
    if (attendance.status !== ATTENDANCE_STATUS.PENDING_ABSENCE) throw ApiError.badRequest('Not pending approval');
